...
## Purpose
Paint what the city is made of: asphalt, pavement, plaza, concrete, and a whole building's outside
(windows, ledges, shopfront, parapet) as one sheet. It draws onto a 2D drawing context you hand it
...
- `paintSurface(kind, ctxFor, opts?) -> SurfacePlan`
  - `kind`: `road` | `pavement` | `plaza` | `concrete` | `facade`.
...
- A facade's albedo and emissive maps are painted from ONE plan, so a window that glows is always a
  window that is there, in the same place, at the same size.
...
palette entry and a line in `GROUND` or a branch in `paintSurface`. Keep `tests/` green: the same
